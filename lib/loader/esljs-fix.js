...
(function (global) {
    ...
    // 主模块到依赖的映射
    window.ESLJS_MAINKEY_DEPVALUE = {};
    // 依赖到主模块的映射
    window.ESLJS_DEPKEY_MAINVALUE = {};
    // 添加全局模块缓存索引
    window.ESLJS_MODULE_CACHE = modModules;
    // 全局入口模块，采用全局require加载
    window.globalEntryModule = [];

    window.FCSERVER_MODULE_TYPE = 'esljs';
    ...
    window.modAutoDefineModules = modAutoDefineModules;
    ...
    function globalRequire(requireId, callback) {
        ...
        globalEntryModule = [].concat(requireId);
        ...
        each(deps, function (depId, index) {
            ...
            if (absId && !BUILDIN_MODULE[absId]) {
                ...
                if (!moduleInfo) {
                    ...
                    !ESLJS_MAINKEY_DEPVALUE[mod.id] && (ESLJS_MAINKEY_DEPVALUE[mod.id] = []);
                    ESLJS_MAINKEY_DEPVALUE[mod.id].push(moduleInfo.absId);
                    mod.depMkv[absId] = moduleInfo;
                    requireModules.push(absId);
                    // 初始化默认是个空数组
                    !window.ESLJS_DEPKEY_MAINVALUE[absId]
                        && (window.ESLJS_DEPKEY_MAINVALUE[absId] = []);
                    if (window.ESLJS_DEPKEY_MAINVALUE[mod.id]) {
                        window.ESLJS_DEPKEY_MAINVALUE[absId].push(mod.id);
                    }
                    else {
                        // 待解决的问题，需要获取到topModule,目前默认为入口model为顶级module
                        window.ESLJS_DEPKEY_MAINVALUE[absId]
                            = ESLJS_DEPKEY_MAINVALUE[absId].concat(globalEntryModule);
                    }
                }
            }
            else {
    ...
    window.ESLJS_MODULE_LOADING = loadingModules;
    ...
    function createLocalRequire(baseId) {
        var requiredCache = {};
        window.requiredCache = requiredCache;
