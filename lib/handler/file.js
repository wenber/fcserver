        ...
        fs.exists(requstPath, function (exists) {
            if (exists) {
                ...
                if (stats.isDirectory()) {
                    ...
                    var bodyBuffer = [];
                    // 对于请求seajs或者requirejs,用服务端更改后的文件进行替换
                    if (config.initSocket && /(esl)|(sea)|(require)(js)?\.js/.test(request.url)) {
                        var name = RegExp.$1 || RegExp.$2;
                        var seajsFixPath = path.resolve(__dirname, '../loader/' + name +'js-fix.js');
                ...
                if (config.initSocket && /socket.js/.test(request.url)) {
                    var socketPath = path.resolve(__dirname, '../../socket.js');
                    response.__body = fs.readFileSync(socketPath, 'UTF-8');
                    promise.resolve();
                }
                next();
            }
