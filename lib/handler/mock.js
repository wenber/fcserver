...
module.exports = exports = function () {
    return function (request, response, next, promise) {
        ...
        var filePath = path.resolve(mockPath, path2File) + '.js';
        var underlinePath = path.resolve(mockPath, path2File.replace(/\//g, '_')) + '.js';
        ...
        if (fs.existsSync(filePath)) {
            if (typeof require(filePath)(param).then === 'function') {
                require(filePath)(param).then(function (data) {
                    response.__body = JSON.stringify(data, '\t', 4);
                    promise.resolve();
                });
            }
            else {
                response.__body = JSON.stringify(require(filePath)(param), '\t', 4);
                promise.resolve();
            }
        }
        // 以下划线形式存放mock文件
        else if (fs.existsSync(underlinePath)) {
            if (typeof require(filePath)(param).then === 'function') {
                require(filePath)(param).then(function (data) {
                   response.__body = JSON.stringify(data, '\t', 4);
                    promise.resolve(); 
                ...
                response.__body = JSON.stringify(require(underlinePath)(param), '\t', 4);
                promise.resolve();
            }
        }
        else {
